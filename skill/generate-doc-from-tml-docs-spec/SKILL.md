---
name: generate-doc-from-template
...
1. **获取文档模板清单**
   ...
   - 如果不存在，从本地或远程获取完整的模板索引（包含所有可用模板的 name、文件路径、类型、适用场景）
   - 模板清单应包含：`Project_Pitch_Spec`、`GitHub_README_Spec`、`Architecture_Design_Spec`、`Domain_Module_Design_Spec`、`Dev_Guidelines_Spec`、`Meeting_Minutes_Spec`、`Team_Resources_Spec`、`Tutorial_Spec`、`Insight_Report_Spec` 等
...
3. **移除模板占位符**
   - 删除所有 YAML Frontmatter（生成的文档中不包含 Frontmatter）
   - 删除所有 `[在此处输出...]` 类型的占位符说明
   - 删除模板中的 `example` 字段内容（仅保留用户的实际内容）
   - 确保最终文档是干净、完整的内容
...
1. YAML Frontmatter 合规性：
   - 生成的文档中是否已完全移除 YAML Frontmatter？
   - 是否理解了原模板中每个 Frontmatter 字段的含义？

2. 内容完整性：
   - 所有 required: true 的章节是否都已填充？
   - 是否删除了所有 [在此处输出...] 占位符？
   - 是否删除了模板中的 example 示例内容？
...
- **严格的格式规范**：YAML Frontmatter 规则、占位符移除、文件命名等都有明确要求，必须严格执行
